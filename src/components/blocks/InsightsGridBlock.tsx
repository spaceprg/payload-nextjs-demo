...
export default function InsightsGridBlock({
  eyebrow,
  heading,
  highlight,
  subtext,
          ...
          {cards.map((card, index) => (
            ...
            >
              ...
              <div className="relative flex flex-col gap-6 rounded-b-lg bg-white/10 p-6">
                <p className="text-sm uppercase tracking-[1.12px] text-lime">{card.category}</p>
                <div className="flex flex-col gap-2 text-white">
                  <div className="flex justify-between text-sm">
                    <span>{card.date}</span>
                    <span>{card.readTime}</span>
                  </div>
                  <p className="text-2xl leading-8">{card.title}</p>
                </div>
              </div>
            </Link>
          ))}
